        ...
        for (int t = 0; t < j; t++) {
          temp = (t == 0) ?
              ...
              : Math.max(temp, dp[i - 1][t - 1] + prices.get(j) - prices.get(
                  t)); //  transaction done on last day = dp[i - 1][t - 1] + prices.get(j) - prices.get(t)
        }
        dp[i][j] = temp;
      }
    }
  ...
  public static int maxProfit(int k, int[] prices) {
    ...
    for (int i = 1; i <= k; i++) {  //  transactions
      for (int j = 1; j < d; j++) {   //  days
        ...
        for (int t = 0; t < j; t++) {
          temp = (t == 0) ?
              Math.max(temp, prices[j] - prices[t])
              //  only one transaction done on last day of the whole cycle
              : Math.max(temp, dp[i - 1][t - 1] + prices[j]
                  - prices[t]); //  transaction done on last day = dp[i - 1][t - 1] + prices.get(j) - prices.get(t)
        }
        dp[i][j] = temp;
      }
    }
